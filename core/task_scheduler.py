...
from .thought import MAX_THOUGHT_SILENCES
...
class SchedulerMixin:
    ...
    def _build_schedule_timing(
        ...
        *,
        ...
        fresh_after_silence: bool = False,
    ) -> dict[str, Any]:
        """统一生成一次调度时间；群聊仍保持上游均匀分布。

        ``fresh_after_silence=True`` 表示心念沉默后重新开启一轮完整的
        min-max 定时区间，而不是在原窗口剩余部分继续重抽。
        """
        ...
        if fresh_after_silence:
            cycle_start = scheduled_at
        else:
            cycle_start = (
        ...
        keep_original_window = after_silence and not fresh_after_silence
        effective_lower = (
            max(distribution.minimum, elapsed_minutes)
            if keep_original_window
            else distribution.minimum
        )
        sampled_minutes = distribution.sample(
            after=effective_lower if keep_original_window else None
        )
        return {
            ...
            "phase": "after_silence" if (after_silence or fresh_after_silence) else "initial",
            "is_private": True,
        }
        ...
        *,
        ...
        if timing["is_private"]:
            ...
            if timing["phase"] == "initial":
                session_payload["thought_silence_count"] = 0
        else:
            for key in (
    ...
    async def _reschedule_after_thought_silence(
        ...
        *,
        ...
        async with self.data_lock:
            ...
            session_payload = self.session_data.setdefault(normalized_session_id, {})
            silence_count = int(
                session_payload.get("thought_silence_count", 0) or 0
            )
            if silence_count >= MAX_THOUGHT_SILENCES:
                return "deadline"
            ...
            session_payload["thought_silence_count"] = silence_count + 1
        ...
        logger.info(
            ...
            f"重新创建一轮 min-max 定时任务至 {run_date.strftime('%Y-%m-%d %H:%M:%S')} "
            f"（第 {silence_count + 1}/{MAX_THOUGHT_SILENCES} 次沉默）喵。"
        )
        return "scheduled"
