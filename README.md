...
- **心念分析模型 (`thought_model`)**:
  - 类型：`String`
  - 默认值：空（复用当前会话主模型）
  - 说明：可选择成本较低、JSON 遵循稳定的小模型，只负责心念判断与话头提取；最终主动消息仍由当前会话主模型生成。
  - 注意：这是特殊配置。通常留空，只有需要降低心念调用成本时再单独指定。

- **精炼心念人格提示词 (`thought_persona_prompt`)**:
  - 类型：`Text`
  - 默认值：空（沿用当前对话实际使用的人格 system prompt）
  - 说明：可为心念小模型单独提供精炼的人格底稿，只需保留身份、关系、语气与边界；不会改变最终主动消息所使用的人格。

- **平均时间位置比例 (`mean_position_ratio`)**:
  - 类型：`Float`
  - 默认值：`0.375`
  - 范围：`0.20 ~ 0.575`
  - 说明：私聊首次心念时间的目标均值为 `最短时间 + 比例 × (最长时间 - 最短时间)`。例如 `40 ~ 200` 分钟配合默认比例时，目标均值为 `100` 分钟。
  - 注意：这是特殊配置，一般不要改；平时只改最短和最长时间，均值会自动跟随。

- **Weibull 分布形状 (`weibull_shape`)**:
  - 类型：`Float`
  - 默认值：`1.7`
  - 范围：`1.5 ~ 2.0`
  - 说明：控制私聊开口时间的聚集程度；数值越大，时间越集中在目标均值附近。
  - 注意：这是特殊配置，一般不要改。

私聊使用截断 Weibull 分布在上下限内安排首次心念。到点后，内置心念层会结合当前会话模型、人格和上下文选择开口或克制；每轮最多克制一次，克制后从原周期剩余时间的条件分布中重抽。第二个时间点不再推迟。群聊仍使用原有的均匀随机调度。
