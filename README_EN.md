...
## ✨ Example Results
  ...
  <img width="750" height="600" alt="Example - Private Chat" src="https://github.com/user-attachments/assets/0996618b-dda8-4788-bf86-eb7f352cb761" />

  <img width="620" height="950" alt="Example - Group Chat" src="https://github.com/user-attachments/assets/a33e0d7c-47ba-48f9-8aac-216f1c941873" />
...
- **Maximum trigger interval (`max_interval_minutes`)**:
  - Type: `Integer`
  - Default: `600`
  - Description: Upper bound of the delay before the next proactive message.

- **Thought analysis model (`thought_model`)**:
  - Type: `String`
  - Default: empty (reuse the current conversation model)
  - Description: Optionally select a lower-cost model for the internal JSON decision and topic extraction. The final proactive message is still generated by the current conversation model.
  - Note: This is an advanced setting. Leave it blank unless you want a separate lower-cost model for inner-thought calls.

- **Compact thought persona prompt (`thought_persona_prompt`)**:
  - Type: `Text`
  - Default: empty (reuse the persona system prompt actually used by the current conversation)
  - Description: Optionally provide the thought model with a compact persona containing only identity, relationship, voice, and boundaries. This does not change the persona used for the final proactive message.

- **Mean position ratio (`mean_position_ratio`)**:
  - Type: `Float`
  - Default: `0.375`
  - Range: `0.20 ~ 0.575`
  - Description: The target mean is `minimum + ratio × (maximum - minimum)`. With `40 ~ 200` minutes, the default ratio gives a `100`-minute target mean.
  - Note: This is an advanced setting. Normally, leave it unchanged and adjust only the minimum and maximum.

- **Weibull shape (`weibull_shape`)**:
  - Type: `Float`
  - Default: `1.7`
  - Range: `1.5 ~ 2.0`
  - Description: Controls how tightly private-chat timings cluster around the target mean.
  - Note: This is an advanced setting and normally should not be changed.

Private chats use a truncated Weibull clock. At the first sampled time, a built-in thought layer may speak or hold back once; after holding back, it resamples from the remaining conditional distribution within the original deadline. Group chats keep the original uniform scheduling behavior.
...
flowchart LR
    ...
    subgraph A[Entry & Static Assets]
        A1[Admin Browser]
        A2[Frontend Entry Page]
        A3[Global Styles & Local Fonts]
        A4[React Runtime]
        A5[Material UI Component Library]
        A6[Plugin Logo Assets]
    end

    subgraph B[Bootstrapping & App Composition]
        B1[Auth Precheck]
        B2[Auth Token Read/Write]
        B3[Browser Local Storage]
        B4[Auth Ready Event]
        B5[Root Mount]
        B6[Auth Wrapper Layer]
        B7[Global State Provider]
        B8[Theme Shell]
        B9[Main App]
    end

    subgraph C[Global State Center]
        C1[Global State Context]
        C2[State Reducer]
        C3[Current View]
        C4[Loading & Error]
        C5[Runtime Snapshot]
        C6[Session List]
        C7[Configuration Snapshot]
        C8[Task List]
        C9[Notifications & Unread State]
        C10[Doc Directory & Content]
        C11[Current Session]
        C12[Theme Mode]
    end

    subgraph D[Data Communication & Real-Time Sync]
        D1[API Call Wrapper]
        D2[Low-Level Request Helper]
        D3[HTTP Requests]
        D4[Realtime Connection Wrapper]
        D5[Global Singleton Connection]
        D6[Listener Dispatch]
        D7[Exponential Backoff Reconnect]
        D8[Initial Full Load]
        D9[Lightweight Polling Fallback]
        D10[Pause Polling When Page Hidden]
    end

    subgraph E[Layout & Global Interaction]
        E1[Sidebar Navigation]
        E2[Top Bar]
        E3[Main Content Area]
        E4[View Switching]
        E5[Theme Toggle]
        E6[Scroll Position Save & Restore]
        E7[Open Plugin Directory]
        E8[Open Data Directory]
        E9[GitHub External Link]
    end

    subgraph F[Five Main Views]
        F1[Runtime Status Page]
        F2[Task Management Page]
        F3[Notification Center Page]
        F4[Documentation Browser Page]
        F5[Configuration Management Page]
    end

    subgraph G[Detailed Page Capabilities]
        G1[Status Cards / Countdown / Manual Refresh]
        G2[Task List / Trigger Now / Cancel Task]
        G3[Notification Sync / Read State Writeback / Body Rendering]
        G4[Directory Load / Document Load / Mermaid Rendering]
        G5[Schema-Driven Form]
        G6[Global Config Mode]
        G7[Per-Session Override Mode]
        G8[Dynamic Field Rendering]
    end

    subgraph H[Shared Utilities & Browser Cache]
        H1[Markdown Renderer]
        H2[Time Formatting Helpers]
        H3[Theme Cache]
        H4[Token Cache]
        H5[Scroll Position Cache]
        H6[Form Draft & Collapse-State Cache]
    end

    subgraph I[Backend APIs]
        I1[Status API /api/status]
        I2[Config API /api/config]
        I3[Config Schema API /api/config-schema]
        I4[Session Config API /api/session-config]
        I5[Task API /api/jobs]
        I6[Notification API /api/notifications]
        I7[Documentation API /api/markdown-files]
        I8[Directory Operation API /api/open-directory]
        I9[Realtime Channel /ws]
    end
