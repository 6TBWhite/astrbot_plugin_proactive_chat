...
- **最大発動間隔 (`max_interval_minutes`)**:
  - 型：`Integer`
  - デフォルト値：`600`
  - 説明：次回自発メッセージの発動範囲における上限値です。

- **心念分析モデル (`thought_model`)**:
  - 型：`String`
  - デフォルト値：空（現在の会話モデルを再利用）
  - 説明：内部 JSON 判断と話題抽出だけに低コストモデルを選択できます。最終的な自発メッセージは引き続き現在の会話モデルが生成します。
  - 注意：これは特殊設定です。心念呼び出しに低コストの別モデルを使う場合以外は空欄のままにしてください。

- **簡潔な心念人格プロンプト (`thought_persona_prompt`)**:
  - 型：`Text`
  - デフォルト値：空（現在の会話で実際に使用される人格 system prompt を再利用）
  - 説明：心念モデル専用に、身元・関係・話し方・境界だけを残した短い人格を指定できます。最終的な自発メッセージの人格には影響しません。

- **平均時間位置の比率 (`mean_position_ratio`)**:
  - 型：`Float`
  - デフォルト値：`0.375`
  - 範囲：`0.20 ~ 0.575`
  - 説明：目標平均は `最短時間 + 比率 × (最長時間 - 最短時間)` です。`40 ~ 200` 分の場合、デフォルト比率で目標平均は `100` 分になります。
  - 注意：これは特殊設定です。通常は変更せず、最短時間と最長時間だけを調整してください。

- **Weibull 分布形状 (`weibull_shape`)**:
  - 型：`Float`
  - デフォルト値：`1.7`
  - 範囲：`1.5 ~ 2.0`
  - 説明：個人チャットの発話時刻が目標平均の周辺にどの程度集中するかを制御します。
  - 注意：これは特殊設定であり、通常は変更しません。

個人チャットは切断 Weibull 時計を使用します。最初の抽選時刻に内蔵の心念レイヤーが発話または一度だけ抑制を選び、抑制後は元の期限内に残る条件付き分布から再抽選します。グループチャットは従来どおり一様ランダムスケジュールを使用します。
