...
class ChatFlowTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_second_silence_reschedules_without_sending(self) -> None:
        harness = _ChatFlowHarness()
        harness.session_data[harness.SESSION_ID]["thought_silence_count"] = 1
        harness.thought_result = {
            "decision": "silent",
            "reason": "还是有点犹豫",
            "topic": "第二次沉默后的话题",
    ...
    async def test_third_silence_cannot_delay_and_uses_latest_topic(self) -> None:
        harness = _ChatFlowHarness()
        harness.session_data[harness.SESSION_ID]["thought_silence_count"] = 2
        harness.thought_result = {
            "decision": "silent",
            "reason": "仍有点犹豫",
            "topic": "第三次强制开口的话题",
        ...
        self.assertIn("第三次强制开口的话题", harness.generated_thought_text)
        self.assertEqual(harness.sent, ["主动消息"])
