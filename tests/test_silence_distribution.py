...
class TruncatedWeibullTests(unittest.TestCase):
    ...
    def test_ratio_tracks_changed_bounds(self) -> None:
        self.assertEqual(target_mean_from_bounds(50, 250, 0.375), 125)

    def test_sampling_clamps_random_source_to_distribution_bounds(self) -> None:
        distribution = build_truncated_weibull(40, 200, 100, 1.7)
        self.assertGreater(distribution.sample(lambda: -1), 40)
        self.assertLessEqual(distribution.sample(lambda: 2), 200)
