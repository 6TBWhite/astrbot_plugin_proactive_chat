...
class SchemaTests(unittest.TestCase):
    ...
    def test_legacy_gate_ui_and_schema_are_gone(self) -> None:
        self.assertNotIn("gate_settings", self.schema["friend_settings"]["items"])
        renderer = (ROOT / "admin/js/components/config/ConfigRenderer.jsx").read_text(
            "utf-8"
        )
        for legacy_key in (
            "gate_settings",
            ...
            self.assertNotIn(legacy_key, renderer)
